...
namespace Mikoto {
    auto ImGuiVulkanBackend::Init( const std::any handle ) -> bool {
        try {
            ...
            if ( vkCreateDescriptorPool( VulkanContext::GetPrimaryLogicalDevice(),
                std::addressof( poolCreateInfo ),
                nullptr,
                std::addressof(  m_ImGuiDescriptorPool ) ) != VK_SUCCESS )
            {
                MKT_THROW_RUNTIME_ERROR( "ImGuiVulkanBackend - Failed to create descriptor pool for ImGui." );
            }
            ...
            ImGui_ImplVulkan_InitInfo initInfo{
                ...
                .PhysicalDevice = VulkanContext::GetPrimaryPhysicalDevice(),
                .Device = VulkanContext::GetPrimaryLogicalDevice(),
                .Queue = VulkanContext::GetPrimaryLogicalDeviceQueuesData().GraphicsQueue,
