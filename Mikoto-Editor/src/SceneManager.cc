...
namespace Mikoto {
    ...
    auto SceneManager::MakeNewScene(std::string_view name) -> Scene& {
        ...
        if ( const auto it{ s_Scenes.find( sceneName ) }; it != s_Scenes.end()) {
    ...
    auto SceneManager::DestroyScene(const Scene& scene) -> void {
        if ( const auto it{ s_Scenes.find( scene.GetName() ) }; it != s_Scenes.end()) {
            s_Scenes.erase(it);
        }
        else {
            MKT_CORE_LOGGER_INFO("SceneManager - Scene {} does not exist.", scene.GetName());
        }
    }
    ...
    auto SceneManager::AddEntityToScene(Scene& scene, const EntityCreateInfo& createInfo) -> Entity {
        ...
        if (!createInfo.IsPrefab()) {
            result = scene.CreateEmptyEntity(createInfo.Name, createInfo.Root);
        }
        else {
            result = scene.CreatePrefabEntity(createInfo.Name, AssetsManager::GetPrefabModel(createInfo.PrefabType), createInfo.Root);
        }
