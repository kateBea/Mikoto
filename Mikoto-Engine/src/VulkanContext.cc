...
namespace Mikoto {
    auto VulkanContext::Init( const std::shared_ptr<Window>& handle ) -> void {
        ...
        // At the moment, Vulkan is working with GLFW windows primarily
        s_ContextData.WindowHandle = std::dynamic_pointer_cast<XPWindow>( handle );
    ...
    auto VulkanContext::CreateSurface() -> void {
        s_ContextData.WindowHandle->CreateWindowSurface( s_ContextData.Instance, std::addressof(s_ContextData.Surface) );
    }
