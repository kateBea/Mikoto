...
// TODO: for this I need to make a workaround for mixing with string views
// Motivation: This right now does not work, I need to convert the view to a string first
// auto do_something(eastl::string_view view) -> void {
//     ankerl::unordered_dense::map<eastl::string, u64> map{};
//     map[view] = 77;
// }
...
namespace ankerl::unordered_dense {
    ...
    struct hash<eastl::string> {
        ...
        auto operator()(eastl::string const& str) const noexcept -> std::uint64_t {
    ...
    template <>
    struct hash<eastl::string_view> {
        using is_avalanching = void;

        auto operator()(eastl::string_view const& sv) const noexcept -> std::uint64_t {
            return detail::wyhash::hash(
                sv.data(),
                sizeof(char) * sv.size()
            );
...
namespace mikoto::string {
    ...
    MKT_NODISCARD
    inline auto ToStd( const eastl::string& str ) -> std::string {
        return { str.c_str(), str.size() };
    }

    MKT_NODISCARD
    inline auto ToEA_Stl( const std::string& str ) -> eastl::string {
        return { str.c_str(), str.size() };
    }
    ...
    MKT_NODISCARD inline auto Trim( eastl::string_view sv ) -> eastl::string_view {
     ...
     * */
    MKT_NODISCARD inline auto Equal( const eastl::string_view str1, const eastl::string_view str2, ComparePolicy policy = ComparePolicy::eIgnoreCase ) -> bool {
        ...
        const eastl::function predicate{
            policy == ComparePolicy::eIgnoreCase ? insensitive : sensitive
        };

        return std::ranges::equal( str1, str2, predicate );
    }

    constexpr auto StartsWith( eastl::string_view str, eastl::string_view other ) -> bool {
    ...
    MKT_NODISCARD inline auto IsNumber( char c ) -> bool {
        if (c < '0' || c > '9')
            return false;
    ...
    MKT_NODISCARD inline auto IsNumber(eastl::string_view value) -> bool {
        return !value.empty() &&
               std::ranges::all_of(value, [](char c) { return IsNumber(c); });
    }
    ...
    template<typename... Args>
    MKT_NODISCARD auto Format(fmt::format_string<Args...> fmt, Args&&... args) -> eastl::string {
        auto result{ fmt::format(fmt, std::forward<Args>(args)...) };

        return eastl::string{ result.c_str() };
    }

    MKT_NODISCARD inline auto From( const eastl::string_view fmt) -> eastl::string {
        return eastl::string{ fmt.data() };
    }
