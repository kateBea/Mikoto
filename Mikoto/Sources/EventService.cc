...
namespace Mikoto {
    ...
    auto EventService::Unsubscribe( const UInt64_T subId, const EventType type) -> void {
    ...
    auto EventService::Unsubscribe( const UInt64_T subId, const EventCategory category) -> void {
        auto it{ m_Subscribers.find(subId) };
        if (it != m_Subscribers.end()) {
            // TODO: maybe change container to erase friendly container
