...
namespace Mikoto {
    ...
    auto PhysicsBase::Shutdown() -> void {
        if (!m_IsInitialized) {
            return;
        }
    ...
    auto PhysicsBase::Update(float dt) -> void {
        PreUpdate();
    ...
    auto PhysicsBase::SetSimulationScene( Scene *scene ) -> void {
        if ( scene ) {
            // This needs to be handled properly
            m_Scene = scene;
        }
    }
    auto PhysicsBase::SetGravity( const Vec3F &gravity ) -> void {
        m_Gravity = gravity;
    }

    auto PhysicsBase::PreUpdate() -> void {
    ...
    auto PhysicsBase::PostUpdate() -> void {
        auto& registry{ m_Scene->GetRegistry() };
    ...
    auto PhysicsBase::OnRigidBodyRemoved( RigidBodyComponent &rb ) -> void {
    ...
    auto PhysicsBase::ToMat4F( const JPH::RMat44 &jphMat ) -> glm::mat4 {
        return glm::mat4{ *reinterpret_cast<const glm::mat4*>(&jphMat) };
    }

    auto PhysicsBase::ToVec3F( const JPH::Vec3 &jphVec3 ) -> glm::vec3 {
        return glm::vec3{ *reinterpret_cast<const glm::vec3*>(&jphVec3) };
    }

    auto PhysicsBase::ToQuatF( const JPH::Quat &jphQuat ) -> glm::quat {
        return glm::quat{ *reinterpret_cast<const glm::quat*>(&jphQuat) };
    }

    auto PhysicsBase::ToVec3( const glm::vec3 &vec3GLM ) -> JPH::Vec3 {
        return JPH::Vec3{ *reinterpret_cast<const JPH::Vec3*>(&vec3GLM) };
    }

    auto PhysicsBase::ToQuat( const glm::vec3 &vec3EulerAnglesGLM ) -> JPH::Quat {
        auto quaternion{ glm::quat( vec3EulerAnglesGLM ) };
        auto ptrQuatJPH{ reinterpret_cast<JPH::Quat*>( std::addressof(quaternion) ) };

        return JPH::Quat{ *ptrQuatJPH };
    }
