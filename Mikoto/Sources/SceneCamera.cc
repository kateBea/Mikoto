...
namespace Mikoto {
    ...
    auto SceneCamera::ProcessMouseInput( const double timeStep ) -> void {
        m_CameraUpVector = Math::GLM_UNIT_VECTOR_Y;
        ...
        if ( delta.x != 0.0f || delta.y != 0.0f ) {
            ...
            const glm::quat rotation{ glm::normalize( glm::cross( glm::angleAxis( -m_Pitch, m_RightVector ), glm::angleAxis( -m_Yaw, Math::GLM_UNIT_VECTOR_Y ) ) ) };
    ...
    auto SceneCamera::UpdateState( const double timeStep ) -> void {
        ...
        m_RightVector = glm::cross( m_ForwardVector, Math::GLM_UNIT_VECTOR_Y );
