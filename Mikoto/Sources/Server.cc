...
namespace mikoto::network {
    ...
    public:
        Session( HttpServer& server, asio::io_context& context )
            : mServer{ server }, mSocket{ context } {}

        auto Start() -> void {
            asio::async_read_until( mSocket, mInput, "\r\n\r\n",
                [self = shared_from_this()]
                ( const asio::error_code& error, const core::usize ) {
                if ( error ) {
                    return;
                }
    ...
    private:
        auto ReadBodyOrDispatch() -> void {
            ...
            if ( headerEnd == std::string::npos ) {
                WriteError( "400", "Bad Request" ); return;
            }

            HttpRequest request{};
            if ( !ParseHttpRequest( eastl::string_view{ raw.data(), raw.size() }, request ) ) {
                WriteError( "400", "Bad Request" ); return;
            }
            ...
            asio::async_read( mSocket, mInput, asio::transfer_exactly( expected - received ),
        ...
        auto Dispatch() -> void {
            ...
            asio::async_write( mSocket, asio::buffer( mOutput.data(), mOutput.size() ),
                [self = shared_from_this()] ( const asio::error_code&, const core::usize ) {
    ...
    HttpServer::HttpServer( NetworkSystem& network )
        : mNetwork{ network } {}
    ...
    auto HttpServer::Listen( const std::string_view address, const core::u16 port ) -> bool {
        if ( !mNetwork.IsInitialized() || port == 0 || IsListening() ) {
            return false;
        }
        ...
        if ( !error ) {
            mAcceptor->set_option( asio::socket_base::reuse_address( true ), error );
        }
        ...
        if ( !error ) {
            mAcceptor->listen( asio::socket_base::max_listen_connections, error );
        }
