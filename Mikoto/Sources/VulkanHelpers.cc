...
#undef MKT_CASE
    ...
    auto GetStageMask( PipelineStageFlags stages ) -> VkPipelineStageFlags2 {
        // All means all queue commands, not every optional extension stage bit.
        if ( stages == PipelineStageFlagsBits::All ) {
            return VK_PIPELINE_STAGE_2_ALL_COMMANDS_BIT;
        }

        VkPipelineStageFlags2 result{ VK_PIPELINE_STAGE_2_NONE };
        if ( stages & PipelineStageFlagsBits::Top ) {
            result |= VK_PIPELINE_STAGE_2_TOP_OF_PIPE_BIT;
        }
        if ( stages & PipelineStageFlagsBits::DrawIndirect ) {
            result |= VK_PIPELINE_STAGE_2_DRAW_INDIRECT_BIT;
        }
        if ( stages & PipelineStageFlagsBits::VertexInput ) {
            result |= VK_PIPELINE_STAGE_2_VERTEX_ATTRIBUTE_INPUT_BIT;
        }
        if ( stages & PipelineStageFlagsBits::IndexInput ) {
            result |= VK_PIPELINE_STAGE_2_INDEX_INPUT_BIT;
        }
        if ( stages & PipelineStageFlagsBits::VertexShader ) {
            result |= VK_PIPELINE_STAGE_2_VERTEX_SHADER_BIT;
        }
        if ( stages & PipelineStageFlagsBits::HullShader ) {
            result |= VK_PIPELINE_STAGE_2_TESSELLATION_CONTROL_SHADER_BIT;
        }
        if ( stages & PipelineStageFlagsBits::DomainShader ) {
            result |= VK_PIPELINE_STAGE_2_TESSELLATION_EVALUATION_SHADER_BIT;
        }
        if ( stages & PipelineStageFlagsBits::GeometryShader ) {
            result |= VK_PIPELINE_STAGE_2_GEOMETRY_SHADER_BIT;
        }
        if ( stages & PipelineStageFlagsBits::PixelShader ) {
            result |= VK_PIPELINE_STAGE_2_FRAGMENT_SHADER_BIT;
        }
        if ( stages & PipelineStageFlagsBits::EarlyFragmentTests ) {
            result |= VK_PIPELINE_STAGE_2_EARLY_FRAGMENT_TESTS_BIT;
        }
        if ( stages & PipelineStageFlagsBits::LateFragmentTests ) {
            result |= VK_PIPELINE_STAGE_2_LATE_FRAGMENT_TESTS_BIT;
        }
        if ( stages & PipelineStageFlagsBits::RenderTarget ) {
            result |= VK_PIPELINE_STAGE_2_COLOR_ATTACHMENT_OUTPUT_BIT;
        }
        if ( stages & PipelineStageFlagsBits::ComputeShader ) {
            result |= VK_PIPELINE_STAGE_2_COMPUTE_SHADER_BIT;
        }
        if ( stages & PipelineStageFlagsBits::Transfer ) {
            result |= VK_PIPELINE_STAGE_2_ALL_TRANSFER_BIT;
        }
        if ( stages & PipelineStageFlagsBits::Bottom ) {
            result |= VK_PIPELINE_STAGE_2_BOTTOM_OF_PIPE_BIT;
        }
        if ( stages & PipelineStageFlagsBits::Host ) {
            result |= VK_PIPELINE_STAGE_2_HOST_BIT;
        }
        if ( stages & PipelineStageFlagsBits::AllGraphics ) {
            result |= VK_PIPELINE_STAGE_2_ALL_GRAPHICS_BIT;
        }
        if ( stages & PipelineStageFlagsBits::AllCommands ) {
            result |= VK_PIPELINE_STAGE_2_ALL_COMMANDS_BIT;
        }
        if ( stages & PipelineStageFlagsBits::Copy ) {
            result |= VK_PIPELINE_STAGE_2_COPY_BIT;
        }
        if ( stages & PipelineStageFlagsBits::Resolve ) {
            result |= VK_PIPELINE_STAGE_2_RESOLVE_BIT;
        }
        if ( stages & PipelineStageFlagsBits::Blit ) {
            result |= VK_PIPELINE_STAGE_2_BLIT_BIT;
        }
        if ( stages & PipelineStageFlagsBits::Clear ) {
            result |= VK_PIPELINE_STAGE_2_CLEAR_BIT;
        }
        if ( stages & PipelineStageFlagsBits::TaskShader ) {
            result |= VK_PIPELINE_STAGE_2_TASK_SHADER_BIT_EXT;
        }
        if ( stages & PipelineStageFlagsBits::MeshShader ) {
            result |= VK_PIPELINE_STAGE_2_MESH_SHADER_BIT_EXT;
        }
        if ( stages & PipelineStageFlagsBits::AccelerationStructureBuild ) {
            result |= VK_PIPELINE_STAGE_2_ACCELERATION_STRUCTURE_BUILD_BIT_KHR;
        }
        if ( stages & PipelineStageFlagsBits::RayTracingShader ) {
            result |= VK_PIPELINE_STAGE_2_RAY_TRACING_SHADER_BIT_KHR;
        }
        return result;
    }

    auto GetAccessMask( BarrierAccessFlags accesses ) -> VkAccessFlags2 {
        VkAccessFlags2 result{ VK_ACCESS_2_NONE };
        if ( accesses & BarrierAccessFlagsBits::StructuredBufferRead ) {
            result |= VK_ACCESS_2_SHADER_STORAGE_READ_BIT;
        }
        if ( accesses & BarrierAccessFlagsBits::IndirectCommandRead ) {
            result |= VK_ACCESS_2_INDIRECT_COMMAND_READ_BIT;
        }
        if ( accesses & BarrierAccessFlagsBits::IndexRead ) {
            result |= VK_ACCESS_2_INDEX_READ_BIT;
        }
        if ( accesses & BarrierAccessFlagsBits::VertexAttributeRead ) {
            result |= VK_ACCESS_2_VERTEX_ATTRIBUTE_READ_BIT;
        }
        if ( accesses & BarrierAccessFlagsBits::ConstantBuffer ) {
            result |= VK_ACCESS_2_UNIFORM_READ_BIT;
        }
        if ( accesses & BarrierAccessFlagsBits::ShaderSampledRead ) {
            result |= VK_ACCESS_2_SHADER_SAMPLED_READ_BIT;
        }
        if ( accesses & BarrierAccessFlagsBits::ShaderStorageRead ) {
            result |= VK_ACCESS_2_SHADER_STORAGE_READ_BIT;
        }
        if ( accesses & BarrierAccessFlagsBits::ShaderStorageWrite ) {
            result |= VK_ACCESS_2_SHADER_STORAGE_WRITE_BIT;
        }
        if ( accesses & BarrierAccessFlagsBits::ColorAttachmentRead ) {
            result |= VK_ACCESS_2_COLOR_ATTACHMENT_READ_BIT;
        }
        if ( accesses & BarrierAccessFlagsBits::ColorAttachmentWrite ) {
            result |= VK_ACCESS_2_COLOR_ATTACHMENT_WRITE_BIT;
        }
        if ( accesses & BarrierAccessFlagsBits::DepthStencilRead ) {
            result |= VK_ACCESS_2_DEPTH_STENCIL_ATTACHMENT_READ_BIT;
        }
        if ( accesses & BarrierAccessFlagsBits::DepthStencilWrite ) {
            result |= VK_ACCESS_2_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT;
        }
        if ( accesses & BarrierAccessFlagsBits::TransferRead ) {
            result |= VK_ACCESS_2_TRANSFER_READ_BIT;
        }
        if ( accesses & BarrierAccessFlagsBits::TransferWrite ) {
            result |= VK_ACCESS_2_TRANSFER_WRITE_BIT;
        }
        if ( accesses & BarrierAccessFlagsBits::ResolveRead ) {
            result |= VK_ACCESS_2_TRANSFER_READ_BIT;
        }
        if ( accesses & BarrierAccessFlagsBits::ResolveWrite ) {
            result |= VK_ACCESS_2_TRANSFER_WRITE_BIT;
        }
        if ( accesses & BarrierAccessFlagsBits::HostRead ) {
            result |= VK_ACCESS_2_HOST_READ_BIT;
        }
        if ( accesses & BarrierAccessFlagsBits::HostWrite ) {
            result |= VK_ACCESS_2_HOST_WRITE_BIT;
        }
        if ( accesses & BarrierAccessFlagsBits::MemoryRead ) {
            result |= VK_ACCESS_2_MEMORY_READ_BIT;
        }
        if ( accesses & BarrierAccessFlagsBits::MemoryWrite ) {
            result |= VK_ACCESS_2_MEMORY_WRITE_BIT;
        }
        if ( accesses & BarrierAccessFlagsBits::AccelerationStructureRead ) {
            result |= VK_ACCESS_2_ACCELERATION_STRUCTURE_READ_BIT_KHR;
        }
        if ( accesses & BarrierAccessFlagsBits::AccelerationStructureWrite ) {
            result |= VK_ACCESS_2_ACCELERATION_STRUCTURE_WRITE_BIT_KHR;
        }
        if ( accesses & BarrierAccessFlagsBits::AccelerationStructureBuildInputRead ) {
