...
namespace Mikoto {
    ...
    static auto SetMaterialTextures( const Scope_T<Mesh> &mesh, PBRMaterialCreateSpec &materialCreateInfo ) -> void {
        for (auto &textureIt: mesh->GetTextures()) {
            switch (textureIt->GetType()) {
                case MapType::TEXTURE_2D_DIFFUSE:
                    materialCreateInfo.AlbedoMap = textureIt;
                case MapType::TEXTURE_2D_SPECULAR:
                    //pbrMaterialCreateSpec.AlbedoMap = textureIt;
                    break;
                case MapType::TEXTURE_2D_NORMAL:
                    materialCreateInfo.NormalMap = textureIt;
                    break;
                case MapType::TEXTURE_2D_METALLIC:
                    materialCreateInfo.MetallicMap = textureIt;
                    break;
                case MapType::TEXTURE_2D_ROUGHNESS:
                    materialCreateInfo.RoughnessMap = textureIt;
                    break;
                case MapType::TEXTURE_2D_AMBIENT_OCCLUSION:
                    materialCreateInfo.AmbientOcclusionMap = textureIt;
                    break;
    ...
    auto Scene::AddEmptyEntity( const std::string_view tagName, const Entity *root ) -> Entity * {
        ...
        if (root == nullptr) { m_Hierarchy.Insert( newEntity.get() ); } else { m_Hierarchy.InsertChild( [&root]( const Entity *parent ) -> bool { return parent->Get() == root->Get(); }, newEntity.get() ); }
