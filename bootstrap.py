...
# From: https://github.com/corporateshark/bootstrapping/tree/master
...
def clone_repository(type, url, target_name, revision, try_only_local_operations = False, recursive = True, target_branch = ""):
    ...
    if type == "hg":
        ...
        branch = f" -b {target_branch}" if len(target_branch) != 0 else ""
        if not repo_exists:
            if try_only_local_operations:
                raise RuntimeError("Repository for " + target_name + " not found; cannot execute local operations only")
            if target_dir_exists:
                ...
                shutil.rmtree(target_dir)
            if recursive:
                dieIfNonZero(executeCommand(TOOL_COMMAND_GIT + " clone --recursive " + url + " " + target_dir + branch))
            else:
                dieIfNonZero(executeCommand(TOOL_COMMAND_GIT + " clone " + url + " " + target_dir + branch))
        elif not try_only_local_operations:
        ...
        try:
            ...
            if source is not None:
                ...
                if src_type == "sourcefile":
                    ...
                    target_branch = source.get('branch', None)
                    ...
                    try:
                        if force_fallback:
                            raise RuntimeError
                        clone_repository(src_type, src_url, name, revision, False, recursive, target_branch)
                        ...
                        if FALLBACK_URL:
                            ...
                            clone_repository(src_type, src_url, name, revision, True, True, target_branch)
                        else:
                            raise
            else:
